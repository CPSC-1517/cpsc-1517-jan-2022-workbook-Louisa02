...
namespace UnitTestSample
{
    ...
    public class EmploymentTests
    {
        ...
        public void CreateEmployment_GoodCreate_EmploymentInstance(string title, SupervisoryLevel level, double years)
        {
            try
            {
        ...
        [DataRow("worker", SupervisoryLevel.Entry, -2.5)]
        public void CreateEmployment_BadCreate_EmploymentInstance(string title, SupervisoryLevel level, double years)
        {
            try
            {
                Employment employment = new Employment(title, level, years);
